...
[comp_acc, parx, pary, parz] = compensator(grndtruth, filt_a, t);
...
%         vel_w3(i:i-1,:) = optiVel(i:i-1,:);
    ...
    az1 = -9.81/(cos(theta) * cos(phi)); % - 0.5 * rssq(R * [optiVel(i-1,1); optiVel(i-1,2); 0]); % + someran(3); %
    kdx1 = 0.57; kdy1 = 0.56;
    ...
    thr_axis = alpha * (-9.81/(cos(theta) * cos(phi))) + (1 - alpha) * newT(i,1); 
