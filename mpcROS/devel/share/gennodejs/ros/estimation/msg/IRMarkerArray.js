...
class IRMarkerArray {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.markers.forEach((val) => {
      bufferOffset = IRMarker.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new IRMarkerArray(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.markers[i] = IRMarker.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.markers.forEach((val) => {
      length += IRMarker.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new IRMarkerArray(null);
    ...
    if (msg.markers !== undefined) {
      ...
      for (let i = 0; i < resolved.markers.length; ++i) {
        resolved.markers[i] = IRMarker.Resolve(msg.markers[i]);
      }
    }
    else {
      resolved.markers = []
